...
public class Triangulation {
	...
	/**
	 * Constructor for creating a Triangulation instance, used to store useful
	 * information needed for completing the algorithm, such as, the number
	 * of data points, the x-coordinates, the y-coordinates, and the solution
	 * array.
	 * 
	...
	public double dLength( int i, int j ) {
		if( j-i >= 2 ) {
			return Math.sqrt( Math.pow(x[i-1]-x[j-1], 2) + Math.pow(y[i-1]-y[j-1], 2) );
		}//end if
		return 0;
	...
	public void triangulation() {
		for( int l = 2 ; l <= size ; l++ ) {
			for( int i = 1 ; i <= size-l+1 ; i++ ) {
				...
				for( int k = i ; k <= j - 1 ; k++ ) {
					double temp = S[i][k] + S[k+1][j] + dLength( i, k ) + dLength( k, j );
					if( temp < S[i][j] ) {
						S[i][j] = temp;
					}//end if
				}//end for k
	 ...
	 * algorithm fills in the solution array and upon completion displays the
	 * result to the user.
	 * 
